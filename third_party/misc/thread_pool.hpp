 ...
 * @version 2.0.0
 * @date 2021-08-14
 * @copyright Modifications Copyright (c) 2022 Wenda Zhou. Copyright (c) 2021 Barak Shoshany.
 * Licensed under the MIT license. If you use this library in published research, please cite it as
 * follows:
...
class thread_pool {
  ...
  public:
    ...
    template <typename F> void push_task(F task) {
        ...
        {
            const std::scoped_lock lock(queue_mutex);
            tasks.push(std::function<void()>(std::move(task)));
        }
    }
